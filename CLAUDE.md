...
v1.1 shipped a WebSocket chat server (`crates/ffoie-chat-server`) with in-memory fan-out, scrollback, rate limiting, and graceful shutdown. The engine client (native + wasm) connects automatically on start and renders chat in the egui HUD. Dev stack: `make docker-up` brings up the chat server (host port `47820`, override via `FFOIE_CHAT_PORT`) plus the wasm engine on host port `47821` (override via `FFOIE_WEB_PORT`); `make soak` runs the 1k-connection 5-minute load test (target overridable with `-e WS_URL=…`). WS endpoint via Docker: `ws://localhost:47820/ws`, health: `curl http://localhost:47820/healthz`. A bare `cargo run -p ffoie-chat-server` (no Docker) still binds the in-container default `8080`.
